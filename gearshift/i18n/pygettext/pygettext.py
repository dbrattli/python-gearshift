...
try:
    from genshi import XML as genshi_parser
    import genshi.core
except:
    genshi_parser = None
...
class TokenEater:
    ...
    def extract_genshi_strings(self):
        """a simple genshi extractor that will just get all the text inside a
        genshi template as long as there is no python code inside it...
        The lien numbers are not reported (nor the tags) and this will just
        show you some nice -1 for the translation context at the moment...
        """
        if not self.__curfile:
            return

        if not genshi_parser:
            raise ImportError, "Genshi templating is not installed."

        for kind, data, pos in genshi_parser(open(self.__curfile, 'r').read()):
            if kind == genshi.core.TEXT:
                item = data.strip()
                if item and not self.__contains_inline_python(item):
                    # TODO: add the context in second arg position by adding 
                    # the full XPATH of the TAG, not by just copy pasting the
                    # kid code below
                    self.__addentry(item, istemplatestring=1)
