...
from gearshift.decorator import decorator
...
If you are stuck, visit http://docs.gearshift.org/GettingHelp for support."""
    if name:
...
def load_class(dottedpath):
    ...
    try:
        ...
        logging.exception('tg.utils: %s not in module: %s' % (classname, modulename))
        raise
    return klass
...
def to_unicode(value):
    ...
    Handles gearshift.i18n.lazystring correctly.
...
def find_precision(value):
    ...
    gearshift.i18n.format.format_decimal() where one has to inform
